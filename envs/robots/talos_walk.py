...
from tasks import talos_walking_task
...
from envs.robots import robot
from .gen_xml_talos import builder


class TalosWalkEnv(mujoco_env.MujocoEnv):
    def __init__(self):
        ...
        path_to_xml_out = '/home/nico/Documents/Talos_Mujoco_Locomotion/models/Talos/tmp/talos.xml'             
        ...
        # set up interface
        self.interface = robot_interface.RobotInterface(self.model, self.data, 'leg_right_6_link', 'leg_left_6_link')
        pdgains = np.zeros((10, 2))
        coeff = 0.5


        pdgains.T[0] = coeff * np.array([200, 200, 200, 250, 80, 
                                         200, 200, 200, 250, 80, ])
        

        pdgains.T[1] = coeff * np.array([20, 20, 20, 25, 8, 
                                         20, 20, 20, 25, 8, ])
        ...
        # set up task
        self.task = talos_walking_task.WalkingTaskTalos(client=self.interface,
                                             dt=control_dt,
                                             neutral_foot_orient=np.array([1, 0, 0, 0]),
                                             root_body='trunk',            # Anpassung: Zentraler Körper (Root)
                                             lfoot_body='leg_left_6_link', # Anpassung: Linker Fuß
                                             rfoot_body='leg_right_6_link' # Anpassung: Rechter Fuß
        )
        ...
        self.actuators = [
            0,  # hip_rotation_l_actuator
            1,  # hip_adduction_l_actuator
            2,  # hip_flexion_l_actuator
            3,  # knee_angle_l_actuator
            4,  # ankle_angle_l_actuator
            5,  # hip_rotation_r_actuator
            6,  # hip_adduction_r_actuator
            7,  # hip_flexion_r_actuator
            8,  # knee_angle_r_actuator
            9   # ankle_angle_r_actuator
        ]

        self.robot = robot.Talos(pdgains.T, control_dt, self.actuators, self.interface)
        ...
        # set action space
        self.action_space_size = len(self.robot.actuators)
        self.action = np.zeros(self.action_space_size)
        self.action_space = np.zeros(self.action_space_size)
