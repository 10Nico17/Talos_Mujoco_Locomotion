...
class JVRC:
    def __init__(self, pdgains, dt, active, client):
        ...
        self.print_actuators()


    def print_actuators(self):
        if hasattr(self.client, "get_motor_names"):
            motor_names = self.client.get_motor_names()
            actuators_with_names = {act_id: motor_names[act_id] for act_id in self.actuators}
        else:
            print("Die Namen der Aktuatoren sind nicht verfügbar, nur IDs werden angezeigt.")
...
class Talos:
    def __init__(self, pdgains, dt, active, client):
        ...
        keyframe_qpos = [
            0, 0, 1.031, 1, 0, 0, 0,  # base position and orientation
            0, 0, 0, -0.3, 0, 0, 0, 0.3, 0, 0.3, 0, 0, 0, -0.3, 0, 0, -0.4,
            0.8, -0.4, 0, 0, 0.4, -0.8, 0.4  # joint angles
        ]

        self.print_actuators()       
        assert len(keyframe_qpos) == self.client.nq()
        self.init_qpos_ = keyframe_qpos
