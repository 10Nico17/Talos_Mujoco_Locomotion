...
"""
def get_normalization_params(iter, policy, env_fn, noise_std, procs=1):
...
def get_normalization_params(iter, policy, env_fn, noise_std, procs=1):
    print("Testing single-process execution without Ray parallelism...")

    try:
        env = WrapEnv(env_fn)
    except Exception as e:
        print(f"Error during environment wrapping: {e}")
        return None, None
    ...
    for t in range(iter):
        try:
            ...
            print(f"Step: {t}, Action before noise: {action}")
            ...
            action = action + torch.randn(action.size()) * noise_std
            print(f"Step: {t}, Action with noise: {action}")
            ...
            print(f"Step: {t}, Next state: {state}, Reward: {reward}, Done: {done}, Info: {info}")

            print("########## 3 ")            


            # Check if action is within the allowed range
            if not env.action_space.contains(action.data.numpy()):
                print(f"Action out of bounds at Step {t}. Action: {action.data.numpy()}, Expected bounds: {env.action_space.low} to {env.action_space.high}")
            ...
            print(f"Error during environment step at Step {t}: {e}")
            break  # Stop execution on error

    
    print("Single-process run completed.")
    return np.mean(states, axis=0), np.sqrt(np.var(states, axis=0) + 1e-8)
